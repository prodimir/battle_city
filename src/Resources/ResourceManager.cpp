...
{
	size_t found = executablePath.find_last_of("/\\"); // find in the Path symbol / or "\\"
...
std::shared_ptr<Renderer::Texture2D> ResourceManager::loadTexture(const std::string& textureName, const std::string& texturePath)
{
	...
	stbi_set_flip_vertically_on_load(true); // OpenGl рисует текстуру(картинку) снизу вверх, а компилятор наоборот. Эта функция компилирует снизу вверх
	...
	stbi_image_free(pixels); // освобождаем место в оперативной памяти из-под текстуры. Сначала мы её загружаем в память видеокарты, потом из ОУ можем удалить
	return newTexture;
}
...
std::shared_ptr<Renderer::Sprite> ResourceManager::getSprite(const std::string& spriteName)
{
	...
	std::cerr << "Can't find the texture program: " << spriteName << std::endl;
	return nullptr;
}